...
v0.3.3 默认根据 GPIO34 已滤波的电池电压持续上升趋势，保守推断“可能正在充电”；
这不是 USB/VBUS 检测。推断成立，或可选 GPIO35 检测确认 USB 存在时，GPIO21
背光强制使用 100% PWM，并停用自动降亮和熄屏。推断状态会在电池电压持续下降后
解除，随后重新开始无操作计时。稳定电压、已经充满，或设备开机前就已插着 USB 时，
电压趋势都无法可靠证明 USB 存在；需要覆盖这些情况时，GPIO35 分压仍是可靠扩展。
...
v0.3.3 还把 Claude 尚未开始的 5h 会话窗口保留为有效窗口：当 `/usage` 明确报告
`0% used` 且没有重置倒计时时，屏幕显示 `剩100%` 和 `重置 未开始`，而不是 `N/A`。
...
- 300 秒没有操作后把 GPIO21 背光 PWM 设为 0。ESP32、触摸和 Wi-Fi 仍在运行，
  因而“熄屏”不是关机或深度睡眠。
- GPIO34 电池电压趋势推断成立，或已安装并启用的 GPIO35 USB +5V 检测确认 USB
  存在时，背光强制使用 100% PWM，不执行前两项省电行为。GPIO34 推断会在持续
  下降趋势后解除；解除或 GPIO35 检测到拔出时，才从零开始计算降亮和熄屏时间。
...
E32R28T 原板没有可直接读取的 USB 存在信号；GPIO34 趋势推断也不能补足稳定、满电
或开机即插电的情况。要获得可靠的 VBUS 存在判断并启用
`external_power_sense_enabled`，须先
...
USB 电源”。E32R28T 原板无法直接检测 USB；GPIO34 电压趋势推断存在上述盲区，
可选 GPIO35 分压虽能可靠判断 VBUS 是否存在，OTA 的人工确认仍然保留，不能用
固件状态代替实际供电检查。设备只接受：
