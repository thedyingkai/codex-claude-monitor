...
> 2026-08-09 当前固件 `native` 15/15 测试通过；`e32r28t` 编译使用
> 100,008/327,680 bytes RAM 和 1,370,753/1,966,080 bytes Flash；旧
> `firebeetle2_esp32s3` 目标也回归编译通过。E32R28T 已通过 COM3 实机烧录、
...
- E32 不会引用旧载板的 GPIO6/GPIO7、MAX17048、USB sense、显示负载开关或双键逻辑；
- 电压-SOC 查表、显示文案、JSON 快照、64KiB 响应限制和 USB sense 旧目标回归共 15 个测试通过；
...
CH340C 上传、屏幕方向、Wi-Fi 与 API 已通过；新背景的最终色彩/可读性、触摸五点、
ADC 标定、电池极性、功耗、温升和续航仍需实物验收。
