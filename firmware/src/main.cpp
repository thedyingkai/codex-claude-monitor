...
const uint8_t brightness_levels[] = {77, 153, 255};
...
void load_config() {
  ...
  config.refresh_seconds = preferences.getUInt("refresh", kDefaultRefreshSeconds);
#if QUOTA_HAS_TOUCH
  ...
  config.refresh_seconds =
      std::clamp<uint32_t>(config.refresh_seconds, 5U, 3600U);
#if QUOTA_HAS_TOUCH
  if (config.touch_x_min + 100U >= config.touch_x_max ||
      config.touch_y_min + 100U >= config.touch_y_max) {
    config.touch_x_min = 495;
    config.touch_x_max = 3398;
    config.touch_y_min = 721;
    config.touch_y_max = 3448;
    last_error = "touch calibration invalid";
  }
#endif
}
...
void save_config() {
  ...
  preferences.putString("base_url", config.base_url);
  preferences.putString("token", config.token);
  preferences.putString("timezone", config.timezone);
  preferences.putUInt("refresh", config.refresh_seconds);
#if QUOTA_HAS_TOUCH
  preferences.putUShort("touch_x0", config.touch_x_min);
  preferences.putUShort("touch_x1", config.touch_x_max);
  preferences.putUShort("touch_y0", config.touch_y_min);
  preferences.putUShort("touch_y1", config.touch_y_max);
#endif
  preferences.end();
  config_dirty = false;
}
...
void persist_snapshot_state(bool force) {
  if (last_accepted_generated_epoch <= 0) return;
  if (force && last_accepted_generated_epoch == last_persisted_generated_epoch &&
      have_persisted_snapshot_cache)
    return;
  if (!force && last_persisted_generated_epoch > 0 &&
      last_accepted_generated_epoch - last_persisted_generated_epoch <
  ...
  const bool cache_ready =
      have_snapshot && quota_monitor::serialize_snapshot_cache(
                           snapshot, cache_json, cache_error);
  preferences.begin("quota-monitor", false);
  bool cache_written = have_persisted_snapshot_cache;
  if (cache_ready) {
    cache_written =
        preferences.putString("snapshot", cache_json.c_str()) == cache_json.size();
  }
  const bool epoch_written =
      preferences.putULong64(
          "last_epoch", static_cast<uint64_t>(last_accepted_generated_epoch)) ==
      sizeof(uint64_t);
  preferences.end();
...
bool config_ready(String& why) {
  if (config.ssid.isEmpty()) {
    why = "SSID missing";
    return false;
  }
  const bool https_url = config.base_url.startsWith("https://");
#if QUOTA_ALLOW_LAN_HTTP
  const bool allowed_lan_http = is_private_lan_http_url(config.base_url);
#else
...
bool fetch_snapshot(String& error) {
  ...
  const auto time_validation = quota_monitor::validate_snapshot_time(
      parsed.generated_at_epoch, now_epoch, last_accepted_generated_epoch,
...
void print_config() {
  ...
  Serial.printf("password=%s\n", masked(config.password).c_str());
  Serial.printf("base_url=%s\n", config.base_url.c_str());
  Serial.printf("token=%s\n", masked(config.token).c_str());
  Serial.printf("timezone=%s\n", config.timezone.c_str());
  Serial.printf("refresh_seconds=%lu\n",
                static_cast<unsigned long>(config.refresh_seconds));
#if QUOTA_HAS_TOUCH
  Serial.printf("touch_cal=%u,%u,%u,%u\n", config.touch_x_min,
                config.touch_x_max, config.touch_y_min, config.touch_y_max);
#endif
...
void set_config_value(const String& key, const String& value) {
  if (key == "ssid")
    ...
    if (key == "touch_x_min") config.touch_x_min = raw;
    if (key == "touch_x_max") config.touch_x_max = raw;
    if (key == "touch_y_min") config.touch_y_min = raw;
    if (key == "touch_y_max") config.touch_y_max = raw;
#endif
  } else {
...
void handle_serial_command(String line) {
  ...
  if (line == "show") {
    ...
    service_wifi();
    String error;
    if (fetch_snapshot(error)) {
      Serial.println("OK snapshot received");
      refresh_ui();
    } else {
...
void change_brightness() {
  ...
  ledcWrite(0, brightness_levels[brightness_index]);
  show_message(String("亮度 ") + String(brightness_percent[brightness_index]) + "%", 2000);
}
...
void show_diagnostics() {
  ...
  if (!last_error.isEmpty()) text += " " + last_error;
...
void show_device_info() {
  ...
  if (have_snapshot) {
    text += "\nC " + local_reset_time(snapshot.codex.observed_at_epoch);
    text += "  A " + local_reset_time(snapshot.claude.observed_at_epoch);
  } else {
    text += "\nC --  A --";
  }
  show_message(text, 6000);
...
void loop() {
  ...
  enter_soft_off();
#endif
  service_button(button_a, manual_refresh, show_diagnostics);
#if QUOTA_HAS_SECOND_BUTTON
  ...
  if (WiFi.status() == WL_CONNECTED &&
    ...
    if (!clock_is_sane()) {
      ...
      if (fetch_snapshot(error)) {
        api_backoff_ms = 1000;
        next_fetch_ms = millis() + config.refresh_seconds * 1000UL;
      } else {
        show_message(error);
